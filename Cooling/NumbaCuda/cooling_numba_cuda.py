...
def run_simulation(
    ...
    with open(csv_file, "w", encoding="utf-8") as csvf:
        ...
        with H5Writer(h5_file, cfg.nx, cfg.ny, batch=32) as writer:
            ...
            stats0 = stats_reducer.finish_host_reduction()
            write_stats_line(csvf, 0, stats0)
            writer.write(0, u_host_pinned)
            output_frames += 1
            ...
            # Close the last empty timing segment if the last action was output.
            pipeline_gpu_ms += stop_gpu_segment_and_sync()
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Numba CUDA cooling solver with robust timing, pinned buffers, and optional CuPy reductions"
    )
